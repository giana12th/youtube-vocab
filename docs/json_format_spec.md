...
- wordfreqのzipfスコアが4.0以上の高頻出語は除外済み（上位約3,000〜5,000語相当）
- 同一単語の重複は除外（最初の出現文を採用）
- zipf < 4.0 の候補が20件未満の場合はその件数をそのまま出力する
...
zipfスコアが低い語を優先する（スコアが低い＝より希少＝より難しい語）。
...
- ADJ 10件をすべて採用
- ADV 8件をすべて採用
- VERB は残り2枠のみ → zipfスコアが低い順に上位2件を採用
